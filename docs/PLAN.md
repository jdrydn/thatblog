...
| --------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `System`        | ElectroDB       | **Stack-wide singleton** `pk=SYSTEM` / `sk=ITEM` (no `blogId`). Session signing secrets (list, for rotation) + a one-time **`setupKey`** (see #18); encrypted at rest, IAM-gated                                                                                                                                                                                       |
...
- [x] Wire Testcontainers + local DynamoDB for unit tests (with James). _(v0.0.2 — `test/global-setup.ts` starts `amazon/dynamodb-local`, creates the full table topology, provides the endpoint to specs via inject.)_
