...
const formatReadItemSchema = z.object({
  ...
  pk: z.tuple([z.literal('BLOGS'), z.uuidv7(), z.literal('POSTS'), z.uuidv7()]),
  // ${contentId}
...
export async function createContentItems(
  blogId: string,
  postId: string,
  creates: Array<{ contentId: string; content: PostContentItem }>,
): Promise<void> {
  ...
  await dcdb.send(
    new TransactWriteCommand({
      TransactItems: creates.map(({ contentId, content }) => ({
        Put: {
          TableName: DYNAMODB_TABLE,
          Item: {
...
export async function updateContentItems(
  blogId: string,
  postId: string,
  updates: Array<{ contentId: string; content: PostContentItem }>,
) {
  ...
  await dcdb.send(
    new TransactWriteCommand({
      TransactItems: updates.map(({ contentId, content }) => ({
        Put: {
          TableName: DYNAMODB_TABLE,
          Item: {
            ...createPostContentItemKey(blogId, postId, contentId),
          ...
          ExpressionAttributeValues: { ':type': content.type },
        },
      })),
    }),
  );
